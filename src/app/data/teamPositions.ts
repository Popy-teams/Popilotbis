...
export function applyPositionsToMember(
  positions: TeamPosition[]
): Pick<TeamMemberFieldsFromPosition, 'role' | 'category' | 'skills'> {
  if (positions.length === 0) return { role: 'Membre', category: 'Non assigné', skills: [] };
  
  return {
    role: positions.map(p => p.title).join(' / '),
    category: positions[0].category,
    skills: Array.from(new Set(positions.flatMap(p => p.competencies))),
  };
}
