...
export function Header({ user, onMenuClick }: HeaderProps) {
  ...
  return (
    <header className="bg-white/95 backdrop-blur border-b border-stone-200 px-4 sm:px-6 py-3 sm:py-4 shrink-0">
      <div className="flex items-center gap-3 sm:gap-4">
        {onMenuClick ? (
          <button
            type="button"
            onClick={onMenuClick}
            className="md:hidden inline-flex items-center justify-center w-10 h-10 -ml-1 bg-stone-900 text-white hover:bg-stone-800 rounded-xl transition-colors shrink-0 shadow-sm"
        ...
        <div className="relative shrink-0 min-w-0 max-w-[min(100%,14rem)] sm:max-w-none">
          ...
          <div className="form-select-wrap rounded-xl border border-stone-200 bg-stone-50/80 min-w-[10rem] sm:min-w-[14rem]">
            <AppIcon icon={FolderKanban} size="sm" className="form-select-icon text-stone-600" />
            <select
              id="active-project"
              value={activeProjectId ?? ''}
              onChange={(e) => setActiveProjectId(e.target.value)}
              className="form-select form-select-with-icon bg-transparent border-0 shadow-none focus:shadow-none text-sm font-semibold text-stone-900 min-h-[2.5rem] py-2 pr-9"
            >
            ...
            <AppIcon icon={ChevronDown} size="xs" className="form-select-chevron text-stone-500" />
          </div>
        </div>
