...
export function LineItemsEditor({
  ...
  const emptyCta = emptyLabel ?? addLabel;
    ...
    <div className="space-y-3">
      ...
      {items.length === 0 ? (
        <button
          type="button"
          onClick={add}
          className="w-full rounded-xl border border-dashed border-indigo-200 bg-indigo-50/40 py-5 text-sm font-medium text-indigo-700 hover:bg-indigo-50 transition-colors inline-flex items-center justify-center gap-2"
        >
...
export function normalizeLineItems(items: string[]) {
